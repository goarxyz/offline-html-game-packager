...
const manualCatalogPath = process.env.MANUAL_CATALOG || "catalog/manual.json";
let manualEntries = [];
try {
  manualEntries = JSON.parse(await readFile(manualCatalogPath, "utf8"));
  if (!Array.isArray(manualEntries)) throw new Error(manualCatalogPath + " must contain an array");
} catch (error) {
...
try {
  await cp("covers", join(siteDirectory, "covers"), { recursive: true });
} catch (error) {
  if (error.code !== "ENOENT") throw error;
}
